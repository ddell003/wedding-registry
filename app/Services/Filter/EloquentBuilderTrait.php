    ...
    private function joinRelatedModelIfExists(Builder $queryBuilder, $key)
    {
        ...
        if (method_exists($model, $key)) {
            ...
            if ($relation instanceof BelongsTo) {
                $tableAlias = $relation->getRelationName();
                $qualifiedForeignKeyName = $relation->getQualifiedForeignKeyName();
                $qualifiedOwnerKeyName = $tableAlias . ".{$relation->getOwnerKeyName()}";

                $queryBuilder->join(
                    $relation->getRelated()->getTable() . " as {$tableAlias}",
                    $qualifiedForeignKeyName,
                    '=',
                    $qualifiedOwnerKeyName,
                    $type
                );
